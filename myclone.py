...
# Ultroid - UserBot
#Edited by @doctorvicky


"""
✘ Commands Available
• `{i}myclone <reply/username>`
...
@ultroid_cmd(pattern="myclone ?(.*)")
async def _(event):
    eve = await eor(event, "`Please wait Doctor Vicky...`")
    ...
    await ultroid_bot.send_message(
        event.chat_id, "**Am I real?**", reply_to=reply_message
    )


@ultroid_cmd(pattern="irevert$")
async def _(event):
    name = OWNER_NAME
    ok = ""
    mybio = str(ultroid_bot.me.id) + '01'
    bio = "Some people only recognize me for that - ain't you the meme creator?"
    chc = udB.get(mybio)
    if chc:
        bio = mybio
    ...
    await eor(event, "You're done Doctor Vicky! Succesfully reverted to your account back !")
